...
人物和场景页面分别使用 `characters.json`、`scenes.json` 以及对应的媒体目录同步图片资产。两页均按动漫/真人和古代/现代分为四个标签页，支持标题与标签搜索；人物额外支持男/女筛选。

新增资产时可以选择文件、拖入图片或直接粘贴图片，并填写标题、标签和分类。取消弹窗不会创建记录或写入图片。图片卡片保持原始比例，点击后可缩放和拖动预览，并可在权限允许时编辑、替换或删除。

人物与场景的编辑和删除遵循与其他内容相同的权限：创建者在 12 小时内可以操作，之后或他人资产需要管理员登录并打开删除权限总开关。
